...
class TestDebtorWebhookService(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.debtor_webhook_service = DebtorWebHooksService(self.mock_client)
        self.mock_auth_header = Mock()
        self.debtor_webhook_service.set_auth_header(self.mock_auth_header)

    def test_delete_webhook(self):
        """Test deleting a webhook for a debtor."""
        self.mock_client.service.WebhookSettings_Delete.return_value = True
        result = self.debtor_webhook_service.delete(1, 123)
        self.assertTrue(result)
        self.mock_client.service.WebhookSettings_Delete.assert_called_once_with(
    ...
    def test_get_webhooks(self):
        ...
        self.mock_client.service.WebhookSettings_Get.return_value = mock_webhooks
        ...
        self.mock_client.service.WebhookSettings_Get.assert_called_once_with(DebtorId=1, _soapheaders=self.mock_auth_header)

    def test_get_webhook_events(self):
        """Test retrieving all webhook events."""
        mock_events = [Mock() for _ in range(3)]
        self.mock_client.service.WebhookSettings_GetEvents.return_value = mock_events
        result = self.debtor_webhook_service.get_all_events()
        self.assertEqual(len(result), 3)
        self.assertTrue(all(isinstance(event, Event) for event in result))
        self.mock_client.service.WebhookSettings_GetEvents.assert_called_once_with(_soapheaders=self.mock_auth_header)
    ...
    def test_insert_webhook(self):
        ...
        self.mock_client.service.WebhookSettings_Insert.return_value = 123
        result = self.debtor_webhook_service.insert(1, mock_webhook_setting)
        self.assertEqual(result, 123)
        self.mock_client.service.WebhookSettings_Insert.assert_called_once_with(
            DebtorId=1,
